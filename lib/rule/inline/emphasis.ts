...
const emphasis: InlineRuleProcessor = (state, silent) => {
  const first_code = state.src.charCodeAt(state.pos);
  if (silent) { return false; }

  if (first_code !== 0x5F /* _ */) {
    return false
  }
...
function process_delimiters(state: InlineState, delimiters: InlineState["delimiters"]) {
  ...
  state.process_pair_delimiter(
    ...
    processor,
    (delimiters) => {
      return delimiters.marker === 0x5F /* _ */ && delimiters.end !== -1
    }
  );
}

export const emphasis_post_processing: InlineRulePPProcessor = (state) => {
